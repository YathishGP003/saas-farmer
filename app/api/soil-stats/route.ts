...
export async function GET(request: Request) {
  ...
  const currentSoilHealth = {
    ...
    texture: 'Loamy',
    moisture: 42, // percentage
    lastUpdated: new Date().toISOString()
  };
  
  // Mock historical soil health data
...
function getHistoricalSoilData(userId: string) {
  ...
  // Generate 6 historical readings (approximately monthly)
  for (let i = 1; i <= 6; i++) {
    const date = new Date(today);
    date.setMonth(date.getMonth() - i);
    
    // Create varied but realistic values
    const phVariance = (Math.random() * 0.6) - 0.3; // ±0.3 pH
    const nutrientVariance = Math.random() * 0.2 - 0.1; // ±10%
...
function generateRecommendations(soilData: any) {
  ...
  // Default recommendation if all values are good
